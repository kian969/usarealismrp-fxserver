...
DoAcid = function(time)
  ...
  while GetGameTimer() - timer < time do
    ...
    if TripInit and not PedSpawned then 
      PedSpawned = true
...
DrawToons = function()
  ...
  if (GetGameTimer() - TripTimer) > 1000 then
    TripTimer = GetGameTimer()
    TripState = TripState + TripAdder

    --if TripState > #Trip.bits then
    --  TripAdder = -1
    --  TripState = 2
    --elseif TripState <= 0 then
    --  TripState = 2
    --  TripAdder = 1
    --end
  end
...
DrawTrip = function(loc)
  local height = 0
  local width = 0

  if TripZOff < 0.0 then TripZOff = TripZOff + TripZAdd; end
  for k = #Trip.bits[TripState],1,-1 do
    local v = Trip.bits[TripState][k]
    local pos = vector3(loc.x,loc.y+width,loc.z+height)
    local col = Trip.cols[v]    

    if TripAlpha < 255 then
      if not TripInit then TripInit = true; end
      TripAlpha = TripAlpha + 0.001
    end

    if v ~= 133 then
      DrawBox(pos.x+0.5,pos.y+0.5,pos.z+0.5 + TripZOff, pos.x-0.5,pos.y-0.5,pos.z-0.5 + TripZOff, col.r,col.g,col.b,math.floor(TripAlpha))
    end

    width = width + 1
    if width > TripLength then
      width = 0
      height = height + 1
    end
  end
