...
local function loadSettings(settings)
    ...
    --exports.globals:notify("HUD settings loaded", "^3INFO: ^0HUD settings loaded")
    Wait(1000)
...
local function restartHud()
    ...
    exports.globals:notify("Restarting HUD", "^3INFO: ^0Restarting HUD")
    ...
    Wait(2600)
    exports.globals:notify("Starting HUD", "^3INFO: ^0Starting HUD")
end
...
RegisterNetEvent("hud:client:resetStorage", function()
    ...
    local menu = TriggerServerCallback {
...
RegisterNetEvent("hud:client:LoadMap", function()
    ...
    if Menu.isToggleMapShapeChecked == "square" then
        ...
        if Menu.isMapNotifChecked then
            --exports.globals:notify("Square map loading", "^3INFO: ^0Square map loading")
        end
    ...
    elseif Menu.isToggleMapShapeChecked == "circle" then
        ...
        if Menu.isMapNotifChecked then
            --exports.globals:notify("Circle map loading", "^3INFO: ^0Circle map loading")
        end
        ...
        Wait(1200)
        if Menu.isMapNotifChecked then
            --exports.globals:notify("Loaded circle map", "^3INFO: ^0Loaded circle map")
        end
    end
end)
...
RegisterNetEvent('hud:client:UpdateNeeds', function(newHunger, newThirst) -- Triggered in qb-core and usa_foodwater
    hunger = newHunger
    thirst = newThirst
end)
...
RegisterNetEvent('seatbelt:client:ToggleSeatbelt', function() -- Triggered in smallresources and usa_seatbelt
...
local function getFuelLevel(vehicle)
    ...
    if (updateTick - lastFuelUpdate) > 2000 then
        ...
        lastFuelCheck = math.floor(exports['usa_fuel']:getFuelLevel())
    end
...
CreateThread(function()
    ...
    while true do
        ...
        --if LocalPlayer.state.isLoggedIn then
            ...
            local voice = 0
            local voiceMode = exports["mumble-voip"]:getVoiceMode()
            if voiceMode == "Downed (Mute)" then
                voice = 0
            elseif voiceMode == "Whisper" then
                voice = 1
            elseif voiceMode == "Normal" then
                voice = 3
            elseif voiceMode == "Shouting" then
                voice = 5
            end
            ...
            if IsPedInAnyVehicle(ped, false) and not IsThisModelABicycle(GetEntityModel(GetVehiclePedIsIn(ped, false))) then
                if exports['LegacyFuel']:GetFuel(GetVehiclePedIsIn(ped, false)) <= 20 then -- At 20% Fuel Left
                    if Menu.isLowFuelChecked then
                        ...
                        exports.globals:notify(Lang:t("notify.low_fuel"), "^3INFO: ^0" .. Lang:t("notify.low_fuel"))
