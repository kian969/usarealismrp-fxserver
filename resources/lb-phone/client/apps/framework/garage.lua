...
RegisterNUICallback("Garage", function(data, cb)
    ...
    elseif action == "toggleLocked" then
        ...
        for i = 1, #vehicles do
            local vehicle = vehicles[i]
            if DoesEntityExist(vehicle) and GetVehicleNumberPlateText(vehicle):gsub("%s+", "") == data.plate:gsub("%s+", "") then
                lockVeh = vehicle
            end
        end
        if data.locked then
            TriggerServerEvent("lock:setLocked", data.plate, false)
            SetVehicleDoorsLocked(lockVeh, 1)
            TriggerEvent("lock:sendNotification", notificationParam, "Vehicle unlocked.", 0.080)
        else
            TriggerServerEvent("lock:setLocked", data.plate, true)
            SetVehicleDoorsLocked(lockVeh, 2)
