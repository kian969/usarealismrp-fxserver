...
function HasPhoneItem(number)
    ...
    local hasPhone = TriggerServerCallback {
...
function ApplyVehicleMods(vehicle, vehicleData)
    if vehicleData.customizations then
        local veh = vehicleData.customizations
...
function CreateFrameworkVehicle(vehicleData, coords)
    ...
    local vehicle = CreateVehicle(numberHash, coords.x, coords.y, coords.z, 0.0, true, false)
