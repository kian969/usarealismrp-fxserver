...
function SetInventoryBusy(isBusy)
    ...
    if isBusy then
        ...
        elseif Framework.Active == 4 then
            --TriggerEvent("interaction:setBusy", isBusy)
            if isBusy then
                TriggerEvent("interaction:forceShutGUI")
            end
        end
    end
...
function CasinoGarage_OnInteraction()
    local ownsCasino = false
    TriggerServerCallback {
        eventName = "properties:ownsProperty",
        args = {"Diamond Casino"},
        callback = function(owns)
            ownsCasino = owns
        end
    }

    if not ownsCasino then
        exports.globals:notify("You don't own the Diamond Casino")
        return
    end
...
RegisterNetEvent("casino:setCasinoVeh", function()
    CasinoGarage_OnInteraction()
end)
