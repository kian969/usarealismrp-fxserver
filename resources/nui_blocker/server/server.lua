...
local logs = GetConvar("nui-blocker-webhook", "")
local kick_msg = "Kicked: Attempted to use nui_devtools!"
local discord_msg = 'Player attempted to use nui_devtools and was kicked.'
local color_msg = 16767235

function sendToDiscord (source,message,color,identifier)
    local name = (GetPlayerName(source) or "UNDEFINED")
    if not color then
        color = color_msg
    end
    local sendD = {
    ...
    PerformHttpRequest(logs, function(err, text, headers) end, 'POST', json.encode({username = "USARRP Logs", embeds = sendD}), { ['Content-Type'] = 'application/json' })
end
