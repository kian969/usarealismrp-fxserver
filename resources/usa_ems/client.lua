...
RegisterNetEvent("lsfd:saveOutfit", function(character)
	-- get name input
	local input = lib.inputDialog('New Outfit', {'Name'})
	if not input then return end
	-- save
	TriggerServerEvent("lsfd:saveOutfit", character, input[1])
end)

RegisterNetEvent("lsfd:showOutfitSelectedMenu", function(id, name)
	...
	outfitsItem.Activated = function(parentmenu, selected)
		...
		for i = 1, #savedOutfits do
			table.insert(outfitMenu, {
				...
				subMenu = true,
				event = "lsfd:showOutfitSelectedMenu",
				args = {
					savedOutfits[i]._id,
					savedOutfits[i].name
				}
